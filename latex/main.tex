...
We use this for our MITM attacks, specifically \code{attack\_INTERCEPT}.
% \textbf{TODO: We're going to use this for an attack}
...
At the end of the day, we found and wrote scripts for 4 unique attacks. They are, in no particuluar order:
\begin{enumerate}
...
\subsection{MITM}
\code{MITM} showcases how, by inserting ourselves between the client and the server, we can fake the server's RSA key-pair to learn the \code{premasterSecret}.
With knowledge of the \code{premasterSecret}, we can decrypt and inspect all traffic between the client and server. Additionally, as the user credentials are sent in this encrypted channel, we learn the user's login as well. This means we can perfectly impersonate the user, so long as we are watching the traffic while the user account is being created.
This \code{MITM} attack is very powerful and constitutes a complete break of the security of the ATM. Undetected, we are able to impersonate a user and perform whatever operations we want on the account, even when the user is not online. 
Additionally, we are able to intercept messages sent to the server and change them to our liking. This attack involves no brute-forcing or complicated analysis, just a simple abuse of the written protocols. This attack is impressive and is explained in detail later in the paper.
